<script lang="ts">
	...
	let hoveredState: { id: string; fill: string; d: string; 'data-info': string } | null = null;
	let mouseCoords = { x: 0, y: 0 };

	export let onSelectedState: (state: string) => void;
</script>

<div
	id="info-box"
	style="display: {hoveredState ? 'block' : 'none'}; top: {mouseCoords.y}px; left: {mouseCoords.x +
		30}px;"
>
	{#if hoveredState}
		{@html hoveredState['data-info']}
	{/if}
</div>
<svg
	x="0px"
...
>
	...
	<g id="g5">
		{#each states as { id, fill, d, "data-info": dataInfo }}
			<a
				data-sveltekit-preload-data="tap"
				href="bills/{id.toLowerCase()}"
				on:mousemove={(event) => {
				...
				on:mouseenter={() => {
					hoveredState = { id, fill, d, 'data-info': dataInfo };
				}}
				on:click={() => onSelectedState(id)}
			>
				<path {id} data-info={dataInfo} {fill} {d} />
			</a>
		{/each}
		<g id="DC">
			<path
